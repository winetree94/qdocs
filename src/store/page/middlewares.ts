...
pageMiddleware.startListening({
  ...
  effect: (_, api) => {
    ...
    const pageGroup = PageSelectors.all(state).reduce<
      Record<EntityId, NormalizedQueueDocumentPage[]>
    >((result, page) => {
    ...
    const changes = Object.values(pageGroup).reduce<
      { id: EntityId; changes: { index: number } }[]
    >((result, pages) => {
